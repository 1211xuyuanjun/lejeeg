...
def train_one_epoch(
        model, time_encoder, freq_encoder, spec_transform, train_loader, 
    ...
    for x in pbar:
        with autocast('cuda', dtype=torch.bfloat16):
            ...
            b = eeg.shape[0]
            # torchaudio Spectrogram 直接输出实数功率谱，无 complex 张量
            power = spec_transform(eeg.flatten(0, 1))
            power = power.reshape(b, -1, *power.shape[1:])
            magnitude_log = 10 * torch.log10(power + 1e-10)  # 10*log10(power) = 20*log10(magnitude)
...
def validate(model, time_encoder, freq_encoder, spec_transform, sigreg, test_loader, config: TrainingConfig, rank: int):
    model.eval()
    time_encoder.eval()
    freq_encoder.eval()
    ...
    with torch.inference_mode():
        for x in pbar:
            with autocast('cuda', dtype=torch.bfloat16):
                ...
                power = spec_transform(eeg.flatten(0, 1))
                power = power.reshape(b, -1, *power.shape[1:])
                magnitude_log = 10 * torch.log10(power + 1e-10)
                spec_embedding = freq_encoder(magnitude_log)
...
def train(model, time_encoder, freq_encoder, sigreg, train_loader, val_loader, config: TrainingConfig, rank: int):
    ...
    spec_transform = torchaudio.transforms.Spectrogram(
        ...
        center=True,
        onesided=True,
        power=2.0,  # 输出功率谱 (magnitude^2)
    ).to(device)

    # 关键：先 DDP 包装，再 torch.compile
    # DDP 在原始模型上注册梯度同步 hook，compile 后 hook 仍然有效
    # 如果先 compile 再 DDP，hook 注册在 OptimizedModule 上，all-reduce 可能不触发
    model = DDP(model, device_ids=[rank])
    time_encoder = DDP(time_encoder, device_ids=[rank])
    freq_encoder = DDP(freq_encoder, device_ids=[rank])

    # DDP 包装后再编译，编译整个 DDP 模块的 forward
    ...
    for epoch in range(config.epochs):
        ...
        validate(model, time_encoder, freq_encoder, spec_transform, sigreg, val_loader, config, rank)
