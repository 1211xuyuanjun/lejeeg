...
def add_time(pos, time_steps):
    batch, channel = pos.shape[:2]
    device = pos.device
    pos_expanded = pos.unsqueeze(2).expand(-1, -1, time_steps, -1)
    t_idx = torch.arange(time_steps, device=device).view(1, 1, -1, 1)
    t_idx = t_idx.expand(batch, channel, -1, 1)
    pos_4d = torch.cat([pos_expanded, t_idx], dim=-1)
    return pos_4d
...
def train_one_epoch(
    ...
    for x in pbar:
        with autocast('cuda', dtype=torch.bfloat16):
            ...
            b = eeg.shape[0]
            eeg_for_stft = rearrange(eeg, 'b c t -> (b c) t')
            spec = torch.stft(
                eeg_for_stft,
                n_fft=127,
                hop_length=8,
            ...
            positions = rearrange(positions, 'b c t n -> n b (c t)')
            time_embedding = rearrange(time_embedding, 'b c t d -> b (c t) d')
            spec_embedding = rearrange(spec_embedding, 'b c t d -> b (c t) d')

            inputs_embeds = torch.stack([time_embedding, spec_embedding], dim=1)
            B, N = inputs_embeds.shape[:2]
            inputs_embeds = rearrange(inputs_embeds, 'b n t d -> (b n) t d')
            
            positions = positions.unsqueeze(2).expand(-1, -1, N, -1)
            positions = rearrange(positions, 'n b rep seq -> n (b rep) seq')
            ...
            proj = rearrange(proj, '(b n) t d -> n b (t d)', b=B, n=N)
