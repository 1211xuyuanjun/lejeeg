...
from qwen3 import Qwen3NextModelJepa, Qwen3NextModelJepaCore, Qwen3NextModelJepaShin2017a, Qwen3NextModelJepaAlex
...
def train_one_epoch(
    ...
    for x in pbar:
        with autocast('cuda', dtype=torch.bfloat16):
            ...
            b = eeg.shape[0]
            positions = positions_origial.expand(b, -1, -1)  # (batch_size, channels, 3)
            spec = torch.stft(
                eeg.flatten(0, 1),
                n_fft=127,
                hop_length=16,
            ...
            logit_loss = F.cross_entropy(logit, y_rep)

            lejepa_loss = sigreg_loss * config.lmab + inv_loss * (1 - config.lmab)
            loss = logit_loss + lejepa_loss
    ...
    with torch.inference_mode():
        ...
        for x in pbar:
            with autocast('cuda', dtype=torch.bfloat16):
                ...
                # 计算每个样本在特征维度上的 L2 范数
                norms = torch.norm(cls, p=2, dim=-1)  # Shape: (batch, seq_len)
                ...
                norm_features = cls / (cls.norm(dim=1, keepdim=True) + 1e-8)
                # 计算余弦相似度：归一化特征的点积
                sim_matrix = torch.mm(norm_features, norm_features.t())
                similarity_mean = sim_matrix.mean().item()
                similarity_std = sim_matrix.std().item()
                

                U, S, V = torch.svd(cls)  # S shape (min(B,D),)
                # 归一化为概率分布
                p = S / S.sum()
                # 计算香农熵
                ...
                wandb.log({
                    ...
                    "l2_norm_min": min_norm,
                    "l2_norm_max": max_norm,
                    "sim_mean": similarity_mean,
                    "sim_std": similarity_std,
                    "eff_rank": eff_rank
                })
        ...
        knn = KNeighborsClassifier(n_neighbors=5, metric='cosine')  # 可用欧氏距离或余弦相似度
...
def main():
    ...
    # 实例化模型
    model = Qwen3NextModelJepaAlex(config=model_config)
    ...
    print("Loading data...")
    # 加载两个已保存的数据集
    windows_dataset = load_concat_dataset(path='/home/xuyuanjun/leeg/data/neural/moabb/Shin2017OpenA/download/EEG_process', preload=True)
